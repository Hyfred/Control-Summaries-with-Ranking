...
def get_response(prompt, model, temperature=0.0):
    '''A function to get the response from LLaMa'''
...
def parsing_llm_keyfact_output(output):

    ''' A function to parse the output from LLMs based on heuristic rules
    Args:
        output: the output from LLMs
    Return: 
        pred_labels: the list include each keyfact
    '''
        
    try:
        output = output.replace('```', '')
        start_idx = output.find('{')
        end_idx = output.rfind('}') # modified
        output = output[start_idx:end_idx + 1] # modified
        output = ast.literal_eval(output)

        matched_lines = set()
        pred_labels = []

        for out in output['key facts']:
...
def get_keyfact_prompt(summary):
    prompt = \
'''
...
def compute_completeness_percentage_score(pred_alignment_labels):
    if pred_alignment_labels==[]: return float('nan') # llm output fail
...
def compute_conciseness_percentage_score(pred_sentence_line_numbers, num_sentences):
    # if pred_sentence_line_numbers==[]: return float('nan') # llm output fail
